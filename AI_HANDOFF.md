...
6. Production remains `WHISPER_BACKEND=mock` and `DIARIZE_BACKEND=mock`. Live GATE-1/2/4 are PASS; GATE-3 candidate v5 (POSTGAP fix) is regenerated + selected — **Peter's re-review is the only open acceptance item**.
7. **Kanban routing changed 2026-08-15:** Programmer and Tester use local `custom:ollama / autoedit-qwen3.8:100k` at explicit `low` reasoning; the 100,000-token load was proved 100% GPU with 11,327 MiB free while Dots was disabled. Publisher remains on `autoedit-qwen3.8:64k`/`low`. Main, auxiliary, and delegation pins are aligned and fallbacks are empty. Tester has browser, computer-use, actual vision, the skills toolset, and focused QA/AUTOEDIT runbooks enabled. Designer is unchanged. Read `docs/status/2026-08-15-kanban-qwen38-routing.md` and `docs/status/2026-08-15-qwen38-shadow-live-handoff.md` before dispatch.
