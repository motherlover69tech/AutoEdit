...
6. Production remains `WHISPER_BACKEND=mock` and `DIARIZE_BACKEND=mock`. Live GATE-1/2/4 are PASS; GATE-3 candidate v5 (POSTGAP fix) is regenerated + selected — **Peter's re-review is the only open acceptance item**. The Tester route is Peter-authorized `custom:9Router / cx/gpt-5.6-luna`, live-smoked as `TESTER_ROUTE_OK`.
7. Stage 7.4 is closed. Historical running/failure card text elsewhere is audit history, not current work. The gateway Chromium/Playwright runtime remains the active Tester browser environment.
