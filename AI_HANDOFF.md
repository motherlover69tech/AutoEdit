...
3. **Current snapshot (2026-08-12 22:45 UTC):** product Phase 5/6 and the 2026-08-12 live-session fixes are shipped/live-verified; the manual Kanban queue is ACTIVE — **Stage 7.4 Tester `t_84759384` is RUNNING** against deployed `ac8407e` (image `sha256:0d6228f7`) after the gateway's missing browser runtime was provisioned (chromium 151 + playwright 1.61.0 + headless shell at `/opt/hermes/.playwright`; smoke-verified as tester uid → loaded `ingest.peteflix.uk` sign-in; card body secret path corrected to `/opt/data/kanban/secrets/autoedit-test-account.json`). Both AUTOEDIT crons remain paused (gateway jobs.json confirmed). A, B, C, D1, D2 are all at review-cap decision points awaiting a Peter call. D1's round-2 correction committed `cb9e4f8` and self-blocked (`t_00d7935c`).
