...
3. **Current snapshot (2026-08-14 01:55 local):** **STAGE 7.4 CLOSED** (Tester v4 `t_e42b0506` TEST_PASS 03:36 UTC; fixes merged `1ba1783`, byte-match production). **AI-GPU-1 LIVE WINDOW COMPLETE (2026-08-13/14)** — full record `docs/status/2026-08-14-live-window-record.md`; checklist updated (`docs/status/2026-08-12-d2-gate4-live-test-checklist.md`). **GATE-1 PASS** (round-2 words accepted by Peter; loudnorm analysis prep `9d823b95…` fixed the quiet-fixture root cause; gate file version-bound, `whisperx_available: True`). **GATE-2 PASS** (both voices re-confirmed for `live-20260814T004600Z`). **GATE-3 READY — Peter's per-window cut review OPEN** (candidate `01KZYT4PWN69CNDY3ZTA7229Q7` selected v4, 278 clips with auditable reasons). **GATE-4 PASS (re-measured)** — overlaps 119 s / 169 s, min real headroom 17,176 MiB ≥ 3,277 (per-process sum 15,592 MiB; the earlier 294 MiB readings = nvidia-smi used-column transients during pyannote churn), Ollama empty at all 8 phase boundaries, 0 restarts, cleanup delta 0 MiB. **Round-6 verdicts:** D1 `t_e8b3accb` DONE (896 passed); B `t_a9049ea5` blocked at final check (856 passed + Node suites); A `t_4b4e8782` blocked (24 focused passed; full-suite stall = external-LLM path); A-Gate1 `t_8487b016` blocked (22 focused passed; commit blocked by git-index permission trap) — no new loops created; Peter's decision pending. D2 remains parked; production stays mock|mock; crons paused. Live-window blockers logged in the checklist + record (incl. `GET /cuts` MySQL sort-buffer 500 → fixed live 16M + persisted config; sampler cross-check vs per-process sums recommended). Browser test-runtime: `browserless-test` on Tower :3002 (token auth; driver shim TBD); gateway chromium+playwright is the active Tester runtime.
...
7. Stage 7.4 targets deployed `ac8407e` / image `sha256:0d6228f7`; the old `master`/`87b9d47` TEST_FAIL is not production evidence. **The Stage 7.4 Tester card is RUNNING as of 2026-08-12 22:44 UTC** — the gateway's browser runtime was provisioned (chromium 151, playwright 1.61.0, headless shell at `/opt/hermes/.playwright`, all smoke-verified as tester uid 10000) and the card body now points at the in-container reviewer secret `/opt/data/kanban/secrets/autoedit-test-account.json`.
