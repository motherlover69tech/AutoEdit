...
3. **Current snapshot (2026-08-13 01:05 UTC):** **Stage 7.4 delete-marker fix is DEPLOYED LIVE** (image `sha256:69f5486d`, player.js `9cc1b6bf` verified in-container, rollback tag `autoedit-rollback:20260813T002300Z` → `0d6228f7`; see `docs/status/2026-08-13-7.4-delete-marker-release.md`). **Tester re-acceptance `t_27b64937` RUNNING on the new image** (dispatcher recovered after a stuck-spawn incident — see BACKLOG; lesson: never re-queue a completed card under its old idempotency key, create a fresh card). **All six acceptance chains (A, A-Gate1, B, C, D1, D2) remain logged at FAILED final checks** — next rounds need Peter; see `docs/status/2026-08-13-morning-review-queue.md`. Both AUTOEDIT crons remain paused (gateway jobs.json confirmed). Browser test-runtime: `browserless-test` on Tower :3002 (token auth; driver shim TBD); gateway chromium+playwright is the active Tester runtime.
