...
3. **Immediate engineering pickup:** Phase 5 residuals (P5-A/B/C) and Phase 6 (speaker-mapping review UI, confirmed-turn projection fix, terminal source-coverage truncation) are SHIPPED; the full stack is integrated to `master` (`17009f7`) AND deployed live (`743d801` api.py + web UI, image `sha256:4d892aee`, verified 2026-08-12). Read `docs/status/phase-6-terminal-truncation-release-2026-08-12.md` (the release handoff): next steps there are (1) the player-tail decision is RESOLVED (disregard tail; player stops at cut end — implemented `743d801`, live), (2) optionally select candidate cut `01KZT3372CDNV7SBKK18STZRZ2` (Peter's call), (3) resume the four AI-GPU-1 application-acceptance gates per `docs/plans/ai-gpu-1-corrective-pickup.md`.
