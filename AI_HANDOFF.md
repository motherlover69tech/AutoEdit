...
- **CURRENT COORDINATION OVERRIDE (2026-08-18):** `docs/stages.json` + `docs/coordinator-state.json` are the sole live coordination authority. Kanban was retired, backed up, and wiped; any Kanban/current-card language later in this file is historical unless reconciled into those JSON files. The active coordinator is `AUTOEDIT JSON state-machine coordinator` every 10 minutes. It refreshes the marker-bounded live pulse in `docs/status/autoedit-progress.html` after every tick via `scripts/update_autoedit_progress_dashboard.py`. Read `docs/COORDINATOR_OPERATING_RULES.md` and `docs/status/AUTOEDIT_PROGRESS_REPORTING.md` before acting.
- **Current state-engine position:** product Stage 8.3 OTIO fallback; latest ordered completion `stage-8.3-red-83-02`; latest independently accepted gate `stage-8.3-green-83-01-tester-check`; one bounded RED-83-02 tests-only correction is being observed after the first Designer rejection. Exact transition time for the already-completed RED-83-02 was not retained by the old state schema; future completions require UTC-stamped `completion_history`.
- **Peter action truth:** no action is ready now. GATE-3 v5 candidate `01KZZRRT8SY5XY5TVN66XF6Z1Y` was rejected; do not ask Peter to re-review it. A reviewed correction must be integrated, deployed, regenerated, and selected as a new exact candidate first. WhisperX rollout remains held for explicit hardware confirmation and a separate rollout decision.
...
3. **Current snapshot (2026-08-14 direct redesign):** **STAGE 7.4 CLOSED. AI-GPU-1 LIVE WINDOW GATE-1/2/4 PASS; GATE-3 rollout/re-review OPEN.** Peter directed the coordinator to bypass Kanban/workers and redesign W1–W6 directly. Local implementation is complete and final verification reports **895 passed / 3 expected integration skips**, redesigned cross-package **216 passed**, both JavaScript suites PASS, Node syntax PASS, Ruff/compileall/diff/shell checks PASS. W1 POSTGAP-R1/R2 passes through the real API path; W2 Package A has validator-owned on-disk authority and path/symlink/forgery regressions; W3 Package B is a version-scoped complete-set batch transaction/UI and one-row swaps fail closed; W4 direct A-Gate1 boundary check passes; W5 D2 is a file-only HMAC-authenticated observer (trusted-host collector not implemented/run here); W6.1–W6.5 pass locally. **Deployed 2026-08-14:** `97149bc` live (image `sha256:44938d4e…`, restarts 0, mock|mock preserved; rollback tag + DB dump + config archive under `release-backups/publisher-97149bc-20260814T090825Z/`); rendered-Compose check passed on the deployment host; W6.1 live (per-connection `SET SESSION sort_buffer_size` + `ix_cuts_project_created_id` index); the 54 superseded round-3..7 kanban cards archived (board now 0 blocked/todo/ready). **GATE-3 candidate `01KZZRRT8SY5XY5TVN66XF6Z1Y` regenerated from the accepted artifact `live-20260814T004600Z` and selected (v5)** — the five post-gap mislabel boundaries now fail safe to `low_confidence:wide`; solo/overlap resolution intact (268 clips). **Remaining: Peter's GATE-3 re-review, then the separate rollout decision.** Production remains mock|mock. The 30-minute whole-pipeline coordinator watchdog is resumed under v7 lane-parking policy: Peter-only tests/decisions are logged as durable `PETER_QUEUE v1` items and excluded from useful-work routing, while independent dependency-safe Kanban work continues. The separate three-hour progress dashboard remains paused. Watchdog audit: `docs/status/2026-08-15-coordinator-watchdog-v7.md`. Source of truth: `docs/status/2026-08-14-postgap-deploy-session.md` (deploy session) + `docs/status/2026-08-14-next-stage-work.md` (design). Browser test-runtime: `browserless-test` on Tower :3002 (token auth; driver shim TBD); gateway chromium+playwright is the active Tester runtime.
...
6. Production remains `WHISPER_BACKEND=mock` and `DIARIZE_BACKEND=mock`. Live GATE-1/2/4 are PASS; GATE-3 candidate v5 (POSTGAP fix) is regenerated + selected — **Peter's re-review is the only open acceptance item in that release lane, but it does not block independent Kanban lanes**.
7. **Role-route assignment updated 2026-08-22 (Peter-directed):** the state-machine **Coordinator** now uses private `custom:9Router / free_wCallback` at **`medium`** reasoning, 200,000 context, and 32,768 maximum output with no fallback (Peter-directed 2026-08-22; live-smoked through the gateway runtime as worker uid); it runs on Peter’s 9Router aggregator (`http://192.168.50.50:20128/v1`) and does NOT consume a Qwen slot. Programmer remains local Qwen at `medium` with aligned main/auxiliary/delegation pins and its `enable_thinking=true` / `parallel_tool_calls=false` provider settings; Publisher remains local Qwen at `medium`. The independent **Tester** now uses private `custom:9Router / cx/gpt-5.6-terra`, 400,000 context, 65,536 maximum output, Terra-aligned delegation/auxiliary routes, and the existing browser, computer-use, vision, and skills capabilities. The Tester must smoke the exact Terra route and actual vision path before a visual verdict. Public OpenRouter remains forbidden; private 9Router is permitted. **Leases are progress-validated (2026-08-20, replaces the 45-min kill clock):** soft reservation — extended +20 min per observe tick while the worker shows progress; terminated only on dead PID / no progress signal for 15 min / 4h hard ceiling. Never terminate a progressing worker on the clock. **Two concurrent Qwen workers are allowed** (Programmer and Publisher only — the Coordinator moved to `custom:9Router / free_wCallback` on 2026-08-22 and the Tester holds Terra, neither consumes a Qwen slot; dispatch while 0–1 slot busy; queue only when both busy). The Qwen stall monitor (`9f2cbee99761`) is **ON** in state-machine form (dead-PID + no-progress-signal detection over sessions/artifacts/slots, 30-min cooldown, silent when healthy); the auto-slicer (`dbbec1c817a0`) remains paused. The state-machine cron `ce71851cfdb7` runs under the **`autoeditcoordinator`** profile (lean clone, 10 skills; gateway multiplex allowlist `[coordinator, autoeditcoordinator]`; `cron.preflight: false` by design).
8. Stage 7.4 is closed. Historical running/failure card text elsewhere is audit history, not current work. The gateway Chromium/Playwright runtime remains the active Tester browser environment.
9. For the broader real-AI phase order, also read `docs/plans/whisperx-speaker-aware-ai-roadmap.md`; the new audiovisual visual-evidence work is planned in `docs/plans/audiovisual-speaker-evidence-plan.md`.
...
- LLM stack fact (2026-08-22): Ollama is RETIRED from the infrastructure. The local model server is llama.cpp at `http://192.168.50.50:8361` (OpenAI-compatible API, Qwen3.8-27b). The deployed app still reaches it through the legacy Ollama-wire shim at `:11435` until job `LLM-CLIENT-LLAMACPP` lands; new code must use `TITLE_BACKEND`/`LLM_BASE_URL`/`LLM_API_KEY`, never add Ollama endpoints or fallbacks. The `OLLAMA_BASE_URL=''` prefix in test commands is only the historical name of the mock-boundary env guard.
