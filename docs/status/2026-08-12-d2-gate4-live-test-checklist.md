...
**2026-08-14 (post-POSTGAP-deploy) state:** candidate `01KZZRRT8SY5XY5TVN66XF6Z1Y` regenerated from the accepted artifact (`live-20260814T004600Z`) with the W1 POSTGAP-R1/R2 fix deployed (master `97149bc`, image `sha256:44938d4e…`) — 268 clips, per-clip auditable reasons; **selected (v5)**; player serves it. The five previously-mislabeled post-gap boundaries (49762, 92135, 205467, 212318, 316690) now fail safe to `low_confidence:wide`; confirmed-solo and overlap regions still resolve correctly. **REVIEW FINDING (Peter, per-window, prior candidate v4):** presenter's post-gap "why are we here" (after "Right" ~46s) labeled SPEAKER_00 → interviewee shot at 49762–60241; root cause was **diarization cluster confusion on post-gap onsets** — addressed by POSTGAP-R1/R2 (onset snapping general + gap classification derived from a sorted timeline; ambiguous crossing-word cases fail safe to wide). **Decision taken: (A) product fix — implemented at `97149bc`, regenerated + selected. Peter's re-review of v5 is OPEN.** Evidence: `20260814-live-window-gate4/browser-review.json` + this session's CDL comparisons.
