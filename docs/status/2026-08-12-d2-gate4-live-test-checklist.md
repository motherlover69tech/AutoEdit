...
**2026-08-14 state:** candidate `01KZYT4PWN69CNDY3ZTA7229Q7` generated from the accepted artifact (`live-20260814T004600Z`) with current cut params — 278 clips, per-clip auditable reasons (`silence:wide`, `speaker:interviewee`, `speaker:presenter`, `overlap:wide`, `interjection:hold`); **selected (v4)** via `PUT /cut-selection` (expected_version 3, mirror CDL atomic replace); previous selection preserved in the cuts table. Player serves the candidate. **Peter's per-window review is OPEN** (watch in the app UI → sign). The remaining checklist items below are the review criteria + the mandatory-window evidence to close on review.
