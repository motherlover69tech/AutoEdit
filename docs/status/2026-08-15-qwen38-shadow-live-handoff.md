...
1. Commit this handoff/routing/backlog/team-policy checkpoint separately on `master`, without staging unrelated dashboard/screenshot/helper artifacts.
2. Archive or comment the six superseded Programmer experiment cards, preserving run history; do not unblock them.
3. Create exactly one Designer compliance-review card against implementation commit `7bcd124`, parented to done Designer spec `t_fc89cea1`.
