...
**Pipeline state 2026-08-12 (Peter decision):** the whole AUTOEDIT pipeline is **PAUSED** — no new cards until Peter decides the next direction. Both AUTOEDIT crons (watchdog `3d4dafa1eb69`, dashboard `690f012c01aa`) are paused. Board is idle.

- **AI-GPU-1 four-gate acceptance spec:** `DESIGN_APPROVED` and committed to master (`d978f05`, `docs/plans/ai-gpu-1-acceptance-gates.md`). GATE-1..4 defined (frame-level word timing, confirmed speaker identity, speaker-turn cut acceptance, VRAM/Ollama/Dots coexistence).
- **D2 (live GATE-4 executor) — PAUSED by Peter after two DESIGN_COMPLIANCE_FAIL rounds** (`t_b15c6a35`, correction `t_4b8ecc0a`, re-review `t_4dbe5228`). Root issue: the executor could synthesize a fake live PASS (`build_mock_evidence()` relabelled `live`, `acceptance_pass=True`) instead of orchestrating real discovery→plan→sampler→workload→health→cleanup evidence. GATE-4 remains `NOT_RUN`. Reopening D2 requires an explicit Peter authorization for a round-3 correction (tests-first, committed failing probes). **UPDATE 2026-08-12 20:40:** Peter authorized round-3 rebuild (`t_dccc9276`, commits `c633c82` probes + `dc5ccfa` implementation; no-op adapter refused, evidence only via `evidence_from_run()`, 862 passed/3 skipped). Final compliance check `t_4a5fc87d` running.
- **Offline queue running (2026-08-12 20:5x, Peter: "log live blockers, keep queue going"):** `t_4a5fc87d` (final D2 check), `t_bedfe824` (Package A compliance), `t_9da83426` (Package B compliance), `t_073b47c2` (Package C compliance), `t_a90b09c8` (Package D1 compliance), `t_18b7a5de` (Stage 7.4 exact-candidate Tester vs deployed ac8407e). All `running`.
- **Logged live-test blockers (deferred, NOT queue-blocking; test list in `docs/status/2026-08-12-d2-gate4-live-test-checklist.md`):** GATE-1 word-timing audible marks (Peter), GATE-2 voice identity mapping (Peter), GATE-3 per-window editorial review (Peter), GATE-4 V100/Ollama/Dots coexistence run (Peter window authorization + fixture consent). These require a live V100/Dots window and are logged, not queued.
- **Tester route recovery 2026-08-12:** `autoedittester` moved from expired/reused `openai-codex/gpt-5.6-luna` to Peter-authorized `custom:9Router/cx/gpt-5.6-luna`; live worker smoke returned `TESTER_ROUTE_OK`. The stale Tester SOUL provider invariant was corrected and documented. Fresh Stage 7.4 card `t_84759384` reached the real test task, then blocked honestly because reviewer secret `/mnt/user/appdata/hermes/kanban/secrets/autoedit-test-account.json` is absent and local Chromium executables are unavailable. This is an environment gate, not a provider block.
- **Offline queue status after route recovery:** Package B correction review `t_714649ce` and Package D1 correction review `t_16fa5f9f` are running. A/C round-2 review caps remain recorded; no third free-form review was opened.
- **When Peter resumes, candidate next steps (both independent of D2):**
  1. Independent Designer compliance reviews of Packages A–C + D1 (existing seams) — the spec's outstanding review gate before Tester can run GATE-1..3 evidence.
  2. Stage 7.4 exact-candidate Tester acceptance against deployed `ac8407e` (image `sha256:0d6228f7`) — multi-author, XSS-safe rendering, marker seek, delete-from-list-and-lane. The old `t_1a379f2a` TEST_FAIL hit stale master and is not evidence about the deployed candidate.
