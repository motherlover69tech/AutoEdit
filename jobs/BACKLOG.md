...
**Current engineering pickup: AI-GPU-1 Phase 5 residual packages from `docs/plans/phase-5-cut-engine-resolved-turns-spec.md`.** Repository inspection at `f2d9d9d` confirms the core resolved-turn→activity bridge, safe-wide cut integration, atomic AI candidate publication, and analysis-status UI are already shipped; do not recreate or re-card them as unstarted. Complete the bounded residuals in order: **P5-A** optional word/speech-quality evidence + complete audit metadata + the named projection test module; **P5-B** explicit `vad`/`whisperx` generation and durable selected-cut backend; **P5-C** non-destructive review-player A/B preview/save UI. After independent compliance and Tester acceptance, the next broader pickup is **Phase 6 — minimal speaker-mapping review UI**. Keep production on `WHISPER_BACKEND=mock` / `DIARIZE_BACKEND=mock` until all four AI-GPU-1 application-acceptance gates and a separate rollout decision pass.
