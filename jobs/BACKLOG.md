...
**Current pipeline state — live snapshot 2026-08-14 01:55 local:** **STAGE 7.4 IS CLOSED** (Tester v4 `t_e42b0506` TEST_PASS 03:36 UTC on image d60d773c; fixes merged `1ba1783`, byte-match production). **AI-GPU-1 LIVE WINDOW COMPLETE (2026-08-13/14)** — full record: `docs/status/2026-08-14-live-window-record.md`; checklist updated: `docs/status/2026-08-12-d2-gate4-live-test-checklist.md`. **GATE-1 = PASS** (round-2 words accepted by Peter; loudnorm analysis prep `9d823b95…` fixed the quiet-fixture root cause; gate file version-bound, `whisperx_available: True`). **GATE-2 = PASS** (both voices re-confirmed for `live-20260814T004600Z`). **GATE-3 = READY — Peter's per-window cut review OPEN** (candidate `01KZYT4PWN69CNDY3ZTA7229Q7` selected v4, 278 clips w/ reasons). **GATE-4 = PASS (re-measured)** — overlaps 119 s/169 s, min real headroom 17,176 MiB ≥ 3,277 (per-process sum 15,592 MiB; the earlier 294 MiB readings = nvidia-smi transients during pyannote churn), Ollama empty at 8 boundaries, 0 restarts, cleanup delta 0. **Round-6 verdicts:** D1 `t_e8b3accb` DONE (896 passed); B `t_a9049ea5` blocked at final check (856 passed + Node suites); A `t_4b4e8782` blocked (24 focused passed; full-suite stall = external-LLM path); A-Gate1 `t_8487b016` blocked (22 focused passed; commit blocked by git-index permission trap) — **no new loops created; Peter's decision pending**. D2 remains parked. Production stays `WHISPER_BACKEND=mock` / `DIARIZE_BACKEND=mock`; crons paused.
...
- **Next manual action:** GATE-3 — Peter reviews the selected WhisperX candidate (`01KZYT4PWN69CNDY3ZTA7229Q7`, v4) in the app UI and signs it. Then decide round-7 for A / A-Gate1 / B (D1 already done) and the D2 rebuild decision. Live window record + evidence pointers in the highlighted checklist + `docs/status/2026-08-14-live-window-record.md`. Crons stay paused.
