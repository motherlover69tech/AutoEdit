...
**Current pipeline state — direct redesign snapshot 2026-08-14:** **STAGE 7.4 CLOSED. AI-GPU-1 LIVE WINDOW GATE-1/2/4 PASS; GATE-3 still requires post-deploy regeneration + Peter re-review.** Peter directed the coordinator to work outside Kanban/workers and redesign W1–W6 directly. The resulting local snapshot is green: POSTGAP-R1/R2 implemented through the real API projection path; Package A replaced with validator-owned on-disk authority; Package B replaced with a version-scoped complete-set batch transaction/UI; the A-Gate1 boundary check passes directly; D2 is now a file-only externally authenticated observer (trusted live collector remains separate); W6.1–W6.5 are implemented. Final pinned suite: **895 passed / 3 expected integration skips**; redesigned cross-package suite **216 passed**; JavaScript, Node syntax, Ruff, compileall, shell syntax, and diff checks pass. **No deploy, live collector run, candidate regeneration, or production re-review has occurred.** Production remains `WHISPER_BACKEND=mock` / `DIARIZE_BACKEND=mock`; crons remain paused. Exact evidence and rollout boundary: `docs/status/2026-08-14-next-stage-work.md`.
...
- **Superseded Kanban history:** the round-3/4/5/6 and D2 cards remain audit records only; none is current work. Their non-PASS findings drove the direct redesign summarized above. Stage 7.4 is closed, and the completed live window established GATE-1/2/4 PASS. The only open acceptance item in this release flow is GATE-3 post-deploy regeneration + Peter re-review. A future automated GATE-4 collector is separate operational follow-up, not a blocker to the existing manual GATE-4 PASS.
...
- **Next manual action:** review the pushed exact commit and migration plan, then use `scripts/autoedit-deploy.sh` for backup → rebuild → verify while retaining `mock|mock`. On the deployment host, run a real rendered-Compose check; then regenerate the GATE-3 candidate from existing accepted inputs and obtain Peter's editorial re-review. A future trusted-host GATE-4 collector remains a separate, explicitly authorized operations item. Crons stay paused.
