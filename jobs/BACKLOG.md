...
**Shipped as of 2026-08-12 (integrated to `master` at `7b0d27f`; frontend deployed live at `743d801`, image `sha256:4d892aee`):** Phase 5 residuals P5-A/B/C (resolved-turn→activity bridge, explicit `vad`/`whisperx` generation + durable selected-cut backend, review-player VAD/WhisperX preview + save UI — DESIGN_COMPLIANCE_PASS `t_e37a9ca4`) and Phase 6 (speaker-mapping review UI `cb3f298`, confirmed-turn projection fix, terminal source-coverage truncation policy — api.py deployed earlier, web UI deployed 2026-08-12, see `docs/status/phase-6-terminal-truncation-release-2026-08-12.md`). Do not re-card any of it. Tester `t_281b2632` returned TEST_PASS on `1d5886a` (45 focused; 851 full / 3 expected skips).

**Next engineering pickup, in order:**
1. **Player tail handling — RESOLVED + DEPLOYED (2026-08-12, Peter decision):** the player disregards the tail and stops when the first feed / cut content ends (implemented `743d801` via `cutEndMs(clips)` + render() pause; live-verified player.js `77c586c9` on Tower). No further work.
2. **WhisperX cut selected as active cut — DONE 2026-08-12 (Peter request):** candidate `01KZT3372CDNV7SBKK18STZR2Z` (correct ID; earlier handoff typo `...STZRZ2`) is now the selected cut (version 2, via `PUT /projects/{id}/cut-selection`, selected_by `autoedit-test`). `edit/cdl.json` mirror rewritten with the WhisperX CDL (truncation present); player opens to the WhisperX cut and stops at 666,167 ms per the tail decision. Reverting = `PUT` the VAD cut `01KXPJJDCFSM377W46WDS1CJZ3` with `expected_version: 2`.
3. **Resume the four AI-GPU-1 application-acceptance gates** (frame-level timing, confirmed speaker identity, speaker-turn cut acceptance, coexistence measurements) per `docs/plans/ai-gpu-1-corrective-pickup.md`. Production stays `WHISPER_BACKEND=mock` / `DIARIZE_BACKEND=mock` until those gates and a separate rollout decision pass.

The separate UI acceptance gate remains Stage 7.4 and may proceed independently. The old exact-`c096e4e` rerun target is now moot — the frontend was redeployed at `743d801` (2026-08-12), so a fresh 7.4-style acceptance (multi-author, XSS-safe rendering, marker seek, delete-from-list-and-lane) should target the new deployed commit `743d801` / image `sha256:4d892aee`. The prior `t_1a379f2a` `TEST_FAIL` hit old `master` at `87b9d47` and is not evidence about any deployed candidate.
