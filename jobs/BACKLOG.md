...
**Current pipeline state — live snapshot 2026-08-14 04:15 local:** **STAGE 7.4 CLOSED**. **AI-GPU-1 LIVE WINDOW: GATE-1/2/4 PASS** (record `docs/status/2026-08-14-live-window-record.md`). **ROUND-7 VERDICTS (2026-08-14, Peter-authorized one-check protocol):** A `t_3d25e9ea` **FINAL FAIL — PARKED** (findings: no direct on-disk regressions; corrected probe has no RED — passes on both ancestors; forged all-PASS still accepted — construction_proof serialized/self-authenticating, unkeyed SHA-256 digest; artifact_valid tautological). B `t_d8dffaa4` **FINAL FAIL — PARKED, do not deploy 080784f** (BACKEND-AIGPU1-003 complete two-row swap still rejected pre-transaction; UI completeness still per-row; no tests/RED added; resolver conflict + UI conflict render PASS). **GATE-3 fix check `t_85c5cd26` FINAL FAIL — PARKED** (POSTGAP-R1: turn-onset snap only in the post-gap branch, not general; POSTGAP-R2: silence-gap classification input-order-dependent; RED + focused 74 + full 861 passed/3 skipped + old-artifact compat PASS) — the candidate `01KZYT4PWN69CNDY3ZTA7229Q7` stays as-is; GATE-3 decision open: accept-with-findings (5 documented boundary-crossing clips) vs park until a new fix approach (the check's findings = the precise spec). **A-Gate1 check re-dispatched** `t_04f4a59e` RUNNING (prior check card t_fe9c14a5 worker silent-exited without a verdict; worktree verified clean, production untouched). Board: C closed, D1 done, A/A-Gate1(?) /B/D2/fix parked; crons paused; production mock|mock. Docs pushed to GitHub through `1d4f1af`.

> **🔴 HIGHLIGHTED TESTING SOURCE OF TRUTH — KEEP THIS UPDATED:** `docs/status/2026-08-12-d2-gate4-live-test-checklist.md`
>
> This is the single living checklist for all future live pushes and acceptance runs. It covers anti-synthetic-PASS checks, GATE-1 frame timing, GATE-2 identity confirmation, GATE-3 speaker-turn cut review, GATE-4 V100/Ollama/Dots coexistence, post-push safety, evidence shape, and rollback. Every future Tester/Publisher/live-acceptance card must link to this file. After any live run, update its checkboxes, exact candidate/image, evidence pointers, verdict, and residual blockers in the same file. Never create a parallel test list.
...
- **D2 / GATE-4:** round-3 implementation is committed (`c633c82` tests-first probes + `dc5ccfa` implementation), but final compliance failed again because the live path/evidence semantics were not yet acceptable. **Peter authorized a FINAL rebuild (2026-08-12 22:50) — card `t_f9b5f857` running** (concrete live executor, no synthetic PASS). GATE-4 remains `NOT_RUN` until that lands.
- **Running now:** **four round-6 Programmer cards** (Peter-authorized 2026-08-13 "Go round-6"): A `t_4b4e8782` (wt/t_5477b373, on 2aa9df4), A-Gate1 `t_8487b016` (wt/t_3cc93a59, on f257f7f), B `t_a9049ea5` (wt/t_9b653567, on 8ecad13), D1 `t_e8b3accb` (wt/t_00d7935c, on 5b31c21) — running since 23:00 UTC, trust-boundary rule + recorded RED, staggered priorities. **D2 remains parked** (multi-day commitment pending a separate decision). Round-5 outcome: **C CLOSED (PASS, tests-only)**; A/A-Gate1/B/D1/D2 FAILED (record: `docs/status/2026-08-13-round5-final-outcomes.md`). **STAGE 7.4 remains CLOSED** (Tester v4 `t_e42b0506` TEST_PASS 03:36 UTC on image d60d773c; fixes merged to master `1ba1783`, byte-matching production). All round-3/4/5 cards remain on the board as the review-cap audit trail.
- **Blocked offline handoffs (audit records, superseded by round-3):** Package C `t_08478be9`, Package A `t_5477b373`, Package A Gate-1 `t_3cc93a59`, Package B `t_9b653567`, Package D1 `t_14396895`, D1 round-2 `t_00d7935c`, and D2 `t_dccc9276` remain blocked as the review-cap audit trail; each now has a Peter-authorized FINAL round-3 card running (see Running now).
- **Stage 7.4:** Tester `t_84759384` completed against deployed `ac8407e` (image `sha256:0d6228f7`) with **TEST_FAIL — BUG-7.4-DELETE-001** (high): authenticated live Chromium run — multi-author rendering, XSS-safe rendering (`window.__xss` stayed 0), and marker seek all PASS; deleting a note removes it from the list but leaves stale `.note-marker` entries in the Notes timeline lane (markerCount unchanged; no console errors). This is the same defect class previously seen at old `87b9d47` — the local c096e4e harness PASS was not authoritative for the lane-marker assertion, so the live defect stands. Correction card `t_fcdda40e` (wt/fix-7.4-delete-marker) running. Browser runtime in the gateway was provisioned 22:44 UTC (chromium 151, playwright 1.61.0, headless shell at `/opt/hermes/.playwright`); card body secret path corrected to `/opt/data/kanban/secrets/autoedit-test-account.json`.
- **Deferred live-test blockers (not queue-blocking):** GATE-1 audible word marks, GATE-2 voice identity decisions, GATE-3 per-window editorial review, and GATE-4 V100/Ollama/Dots coexistence authorization/fixture. Track all of them only in the highlighted checklist above.
- **Tester route recovery:** `autoedittester` moved from expired/reused `openai-codex/gpt-5.6-luna` to Peter-authorized `custom:9Router/cx/gpt-5.6-luna`; config, SOUL, and routing baseline are aligned and the actual worker smoke passed.
- **Next manual action:** (1) GATE-3 decision — accept the current candidate with the 5 documented post-gap misclassification clips (findings + evidence in `20260814-live-window-gate4/browser-review.json`), or park GATE-3 pending a new fix approach (the check's POSTGAP-R1/R2 findings = the precise spec for a fresh card; resurrection needs a new Peter decision). (2) A-Gate1 check verdict (`t_04f4a59e` running — re-dispatch after the silent-exit). (3) D2 decision whenever ready. A and B are parked permanently per the one-check protocol. Live window record + evidence pointers in the highlighted checklist + `docs/status/2026-08-14-live-window-record.md`. Crons stay paused.
...
**Next engineering pickup, in order:**
1. **Player tail handling — DONE + DEPLOYED + VERIFIED (2026-08-12).** Player disregards the tail and stops when the first feed / cut content ends (`cutEndMs(clips)` + render() pause). No further work.
2. **WhisperX cut selected as active cut — DONE + VERIFIED (2026-08-12, Peter request):** candidate `01KZT3372CDNV7SBKK18STZR2Z` (correct ID; earlier handoff typo `...STZRZ2`) is the selected cut (version 2, project "sm test cab" `01KXPHM8XCBKZ96Y2JN6T9Q2MC`). Reverting = `PUT` the VAD cut `01KXPJJDCFSM377W46WDS1CJZ3` with `expected_version: 2`.
3. **Cut-review preview/save UX — DONE + DEPLOYED + VERIFIED (2026-08-12):** all 8 param controls now reach the cut engine on regenerate; "Save this cut" works (expected_version read from `cut.selection_version`); API errors render as readable text instead of `[object Object]`.
4. **Resume the four AI-GPU-1 application-acceptance gates** (frame-level timing, confirmed speaker identity, speaker-turn cut acceptance, coexistence measurements) per `docs/plans/ai-gpu-1-corrective-pickup.md`. Production stays `WHISPER_BACKEND=mock` / `DIARIZE_BACKEND=mock` until those gates and a separate rollout decision pass.

The separate UI acceptance gate remains Stage 7.4 and may proceed independently. The old exact-`c096e4e` rerun target is now moot — the frontend was redeployed through `ac8407e` (2026-08-12, image `sha256:0d6228f7`), so a fresh 7.4-style acceptance (multi-author, XSS-safe rendering, marker seek, delete-from-list-and-lane) should target the deployed commit `ac8407e` / image `sha256:0d6228f7`. The prior `t_1a379f2a` `TEST_FAIL` hit old `master` at `87b9d47` and is not evidence about any deployed candidate.

Final deterministic local checkpoint on the integrated tree: run `OLLAMA_BASE_URL='' LLM_MODEL='' env -u VIRTUAL_ENV uv run pytest -q -rs` (Tester's `1d5886a` run: 851 passed / 3 skipped; the skips are credential-gated central-MySQL + Ollama-gated integration tests). V100 `/ready`, queued real ASR/alignment and constrained diarization, and the audit-only Qwen context pass succeeded; the four application-acceptance gates above remain open.
...
- **Status:** done — **CLOSED 2026-08-13 03:36 UTC: Tester v4 `t_e42b0506` TEST_PASS** on image `sha256:d60d773c` (live authenticated Chromium: multi-author, XSS-safe, marker seek, delete-from-list-and-lane, responsive width grid + exact geometry — all green). Fix history: BUG-7.4-DELETE-001 (stale lane markers after delete) fixed + deployed 00:23 (player.js 3728b71/bfe6a5f, image 69f5486d, accepted by Tester v3); BUG-7.4-UI-RESPONSIVE-001 (horizontal overflow ≤800px + 1024/wide geometry) fixed after 5 review/correction rounds (styles.css 232c551, image d60d773c, deployed 03:32, byte-verified d87c8f5c). Both fixes merged to master `1ba1783`.
...
- **Stage 7.4:** ✅ **CLOSED (2026-08-13)** — Tester v4 TEST_PASS on image d60d773c; delete-marker + responsive fixes deployed and merged to master `1ba1783`. Full record: `docs/status/2026-08-13-7.4-delete-marker-release.md` + morning-review queue.
