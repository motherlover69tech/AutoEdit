...
**Current pipeline state — live snapshot 2026-08-13 01:05 UTC:** the manual Kanban queue is **ACTIVE with ONE card running: Tester re-acceptance `t_27b64937`** (Stage 7.4 on the new live image). **Stage 7.4 delete-marker fix DEPLOYED LIVE 00:23 UTC** (image `sha256:69f5486d`, coordinator-direct after the Publisher worker silently exited twice without reporting; fully verified: player.js `9cc1b6bf` byte-identical to the reviewed fix, /health ok, /projects 401, backup + rollback tag). **Dispatcher incident (00:21–01:01, RESOLVED):** after the zombie Publisher run the gateway dispatcher stopped spawning workers ("dispatcher stuck: 0 workers spawned"); a re-queued card with a completed-run idempotency key was never claimed; fix = clear terminal-state fields (completed_at/result/started_at/current_run_id) on re-queue OR create a fresh card with a new idempotency key (this worked), plus `s6-svc -r` on gateway-coordinator via `/command/s6-svc`. Both AUTOEDIT crons remain paused (gateway jobs.json confirmed). **All six acceptance chains (A, A-Gate1, B, C, D1, D2) remain logged at FAILED final checks** — see `docs/status/2026-08-13-morning-review-queue.md` (the consolidated Peter queue; no further loop without Peter). Browser test-runtime: `browserless-test` on Tower :3002 (token auth in ops-stack/browserless/compose.yaml; driver shim TBD); gateway chromium+playwright is the active Tester runtime. Do not infer state from the dashboard HTML.
...
- **Running now:** SEVEN cards (all with live worker PIDs): round-3 rebuilds A `t_4febc871`, A-Gate1 `t_cb208061`, B `t_3786272d`, C `t_83e29173`, D1 final check `t_b24df767`, D2 `t_f9b5f857` (Peter-authorized FINAL round-3, 2026-08-12 22:50), plus 7.4-delete correction `t_fcdda40e`. Stage 7.4 Tester `t_84759384` returned **TEST_FAIL — BUG-7.4-DELETE-001** (stale `.note-marker` in Notes lane after delete; live Chromium evidence, authenticated reviewer).
...
- **Next manual action:** none required right now — seven cards are running (six Peter-authorized FINAL round-3 acceptance rebuilds + the 7.4 delete-marker correction). When round-3 cards self-block with READY_FOR_REVIEW, create the single final compliance check per chain (FINAL, no further loop without Peter). When the 7.4 fix lands, review → Publisher deploy → Tester re-run. Do not resume the AUTOEDIT crons.
