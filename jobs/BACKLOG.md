...
- **Kanban worker route (current, 2026-08-15):** `autoeditprogrammer`, `autoedittester`, and `autoeditpublisher` now use local `custom:ollama / autoedit-qwen3.8:64k` with a server-side and Hermes-declared 65,536-token context. Main/auxiliary/delegation pins are aligned, fallbacks are empty, and live Ollama tool-call + `/api/ps` context/VRAM checks passed. Designer is unchanged. Historical Codex/Luna/Tester-9Router route records are superseded; see `docs/status/2026-08-15-kanban-qwen38-routing.md`.
